...
| `DefaultNFATAssembler` | A PAU stack via `PAUAssembler` + an NFAT facility wired to the resulting shared `ALMProxy`.       | [README](./DefaultNFATAssembler/README.md)           | [CHECKLIST](./DefaultNFATAssembler/CHECKLIST.md) _(draft)_   |
...
- **`DefaultNFATAssembler`** composes **`PAUAssembler`**: it forwards the full PAU configuration, then binds an NFAT facility's `recipient` and sole `bud` to the shared proxy. The PAU half of its review is covered by the `PAUAssembler` checklist.
